...
void mouseClicked() {
  //print(mouseX, mouseY,"                            ");
  if(g.on == true && g.pause == true && g.currentchar.tired == false){
    for(int i = 0; i < 6; i++){
      if(mouseX > i*1000/6 && mouseX <(i+1)*1000/6){g.currentchar.xloc = i;}
      }
    for(int j = 0; j <4; j++) {
      if(mouseY > 800/5 + (j)*800/5 && mouseY <((j+1)*800/5 + 800/5)){g.currentchar.yloc = j;}}
      Tile r = g.tiles[59+g.currentchar.xloc][g.currentchar.yloc];
      print(g.currentchar.xloc,g.currentchar.yloc,r.type,r.activity,"\n");
